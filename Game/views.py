...
from .models import User, Admin, Guest, Quiz, Country, Answer, QuizDone
...
def accountGuest(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			messages.error("Please try to change username and make password that contains atleast 8 characters and numbers")
	else:
		form = UserCreateForm()
	context = {"form":form}
...
def playCountry(request, pk):
	...
	if request.method == "POST":
		...
		answer = Answer.objects.create(name=name, is_country=is_country, is_true=is_true,quiz=quiz_instance,country=country_instance,user=user_instance)
